...
Iterable<LocaleInfo> localesForLanguage(Iterable<LocaleInfo> locales, String language) =>
    locales.where((LocaleInfo element) => element.languageCode == language);
...
String _generateLookupBody(
  ...
  return lookupBodyTemplate
      .replaceAll(
        ...
        _generateLookupByAllCodes(locales, generateSwitchClauseTemplate),
      )
      .replaceAll(
        '@(lookupScriptCodeSpecified)',
        _generateLookupByScriptCode(locales, languages, generateSwitchClauseTemplate),
      )
      .replaceAll(
        '@(lookupCountryCodeSpecified)',
        _generateLookupByCountryCode(locales, languages, generateSwitchClauseTemplate),
      )
      .replaceAll(
        '@(lookupLanguageCodeSpecified)',
        _generateLookupByLanguageCode(locales, languages, generateSwitchClauseTemplate),
      );
}
...
class LocalizationsGenerator {
  ...
  late final _groupCollection = AppResourceGroupCollection(inputDirectory);
  late final Iterable<LocaleInfo> _supportedLocales = _groupCollection.supportedLocales;
  late final Iterable<String> _languages = _supportedLocales.map(
    (LocaleInfo locale) => locale.languageCode,
  );

  late final _inputFileNames = Map<LocaleInfo, String>.fromEntries(
    _groupCollection.allBundles.map(
  ...
  ///
  /// It is assumed that all input files (e.g. arb files
  ...
  /// generated class that's written to [baseOutputFile].
  ...
  List<String> get arbPathStrings {
    return _groupCollection.allBundles.map((AppResourceBundle bundle) => bundle.file.path).toList();
  }
  ...
  /// Sets the reference [File] for the template ARB file.
  @visibleForTesting
  ...
  void loadResources() {
    final Iterable<AppResourceBundle> templates = _groupCollection.bundlesForLanguage(
      templateLocale,
    );
    ...
    for (final template in templates) {
      for (final String resourceId in template.resourceIds) {
        if (!_isValidGetterAndMethodName(resourceId)) {
          throw L10nException(
            'Invalid ARB resource name "$resourceId" in ${template.file.path}.\n'
      ...
      final AppResourceBundleCollection bundleCollection = _groupCollection.bundleForNamespace(
        namespace,
      );
      final Iterable<Message> templateMessages = template.resourceIds.map(
        (String id) => Message(
          template,
          bundleCollection,
          id,
    ...
    if (inputsAndOutputsListFile != null) {
      _addAllToFileList(
        _inputFileList,
        _groupCollection.allBundles.map((AppResourceBundle bundle) {
          return bundle.file.absolute.path;
        }),
      );
    }

    final allLocales = List<LocaleInfo>.from(_supportedLocales);
  ...
  String _generateBaseClassFile(
    ...
    final Iterable<String> methods = _allMessages.map((Message message) {
      ...
      if (message.messages[locale] == null) {
        _addUnimplementedMessage(locale, message.formattedResourceId);
